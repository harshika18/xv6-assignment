...
int main(int argc , char *argv[])
{
  ...
  1c:	0f 8e 9a 00 00 00    	jle    bc <main+0xbc>
    {
        n = 1;
    }
    else
  ...
  28:	e8 b3 02 00 00       	call   2e0 <atoi>
    }
    if(argc < 3)
  ...
  3d:	e8 9e 02 00 00       	call   2e0 <atoi>
  ...
  60:	74 6b                	je     cd <main+0xcd>
        {
            printf(1, "Parent %d creating child %d\n", getpid(),id);
  62:	e8 6b 03 00 00       	call   3d2 <getpid>
  67:	56                   	push   %esi
  68:	50                   	push   %eax
    {
        d = atoi(argv[2]);
  ...
  82:	7d 2c                	jge    b0 <main+0xb0>
    {
        id = fork();
  84:	e8 c1 02 00 00       	call   34a <fork>
        if( id < 0)
  ...
  8d:	79 d1                	jns    60 <main+0x60>
        {
            printf(1, "%d failed in fork\n",getpid());
  8f:	e8 3e 03 00 00       	call   3d2 <getpid>
  ...
  ae:	66 90                	xchg   %ax,%ax
    ...
    exit();
  b0:	e8 9d 02 00 00       	call   352 <exit>
    {
        n = atoi(argv[1]);
    }
    if(argc < 3)
    {
        d = 1.0;
  b5:	d9 e8                	fld1   
  b7:	dd 5d d0             	fstpl  -0x30(%ebp)
  ba:	eb 92                	jmp    4e <main+0x4e>
    }
    else
    {
        d = atoi(argv[2]);
  ...
  c3:	dd 5d e0             	fstpl  -0x20(%ebp)
    ...
    if(argc < 3)
    {
        d = 1.0;
  c6:	d9 e8                	fld1   
  c8:	dd 5d d0             	fstpl  -0x30(%ebp)
  cb:	eb 8a                	jmp    57 <main+0x57>
            printf(1, "Parent %d creating child %d\n", getpid(),id);
            wait();
        }
        else
  ...
  cd:	e8 00 03 00 00       	call   3d2 <getpid>
  d2:	52                   	push   %edx
  d3:	50                   	push   %eax
  d4:	68 10 08 00 00       	push   $0x810
  d9:	6a 01                	push   $0x1
  ...
  e5:	dd 05 28 08 00 00    	fldl   0x828
  eb:	eb 05                	jmp    f2 <main+0xf2>
  ed:	8d 76 00             	lea    0x0(%esi),%esi
  f0:	d9 c9                	fxch   %st(1)
  f2:	dd 45 e0             	fldl   -0x20(%ebp)
  f5:	d8 c1                	fadd   %st(1),%st
  f7:	dd 5d e0             	fstpl  -0x20(%ebp)
  fa:	d9 c9                	fxch   %st(1)
            wait();
        }
        else
        {
            printf(1, "Child %d created\n",getpid());
            for(z = 0; z < 8000000.0 ; z += d)
  fc:	dc 45 d0             	faddl  -0x30(%ebp)
  ff:	d9 05 30 08 00 00    	flds   0x830
 105:	df e9                	fucomip %st(1),%st
 107:	77 e7                	ja     f0 <main+0xf0>
 109:	dd d8                	fstp   %st(0)
 10b:	dd d8                	fstp   %st(0)
 10d:	eb a1                	jmp    b0 <main+0xb0>
 10f:	90                   	nop
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 45f:	f7 f6                	div    %esi
 461:	0f b6 92 3c 08 00 00 	movzbl 0x83c(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5d8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5db:	b8 34 08 00 00       	mov    $0x834,%eax
